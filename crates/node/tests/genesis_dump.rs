...
fn dump_full_genesis() {
    let genesis =
        rxrpl_node::Node::genesis_with_funded_account("rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh")
            .unwrap();
    let key = rxrpl_protocol::keylet::account(
        &address::decode_account_id("rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh").unwrap(),
    );
    let sle_bytes = genesis.get_state(&key).unwrap();
    eprintln!(
        "FULL_GENESIS account_hash={} hash={} sle_len={} sle={}",
        genesis.header.account_hash,
        genesis.header.hash,
        sle_bytes.len(),
        sle_bytes
            .iter()
...
fn dump_ledger_2_with_skip_list() {
    ...
    eprintln!(
        "LEDGER_2 account_hash={} hash={}",
        l2.header.account_hash, l2.header.hash
    );
...
fn dump_via_new_standalone() {
    ...
    let node =
        rxrpl_node::Node::new_standalone(config, "rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh").unwrap();
    ...
    if let Some(g) = closed.iter().find(|l| l.header.sequence == 1) {
        ...
        match g.get_state(&key) {
            Some(b) => eprintln!(
                "VIA_NEW_STANDALONE account_hash={} hash={} sle_len={} sle={}",
                g.header.account_hash,
                g.header.hash,
                b.len(),

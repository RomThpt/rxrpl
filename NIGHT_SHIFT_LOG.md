...
[2026-04-28T00:30:00Z] [night-coder] T30 STValidation alloc cap: added MAX_STVALIDATION_BYTES=32 KiB const, capped Vec::with_capacity at .min(MAX_STVALIDATION_BYTES), upfront reject when payload.len() > 2*MAX, +1 regression test (16 MiB blob rejected). cargo test -p rxrpl-overlay green (242 tests).
[2026-04-28T01:30:00Z] [night-tester] T31 stale-validation replay: +3 integration tests in crates/consensus/tests/validations_trie_stale_replay.rs (stale_seq_validation_rejected_after_newer, same_seq_older_sign_time_rejected, same_seq_same_sign_time_idempotent) pinning audit pass 2 C1 monotonicity contract. cargo test -p rxrpl-consensus green: 228 lib + 3+3+3 integration, 0 failed.
